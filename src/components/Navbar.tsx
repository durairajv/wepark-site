...
export default function Navbar() {
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 lg:px-10 flex items-center justify-between">
        ...
        <a href="#" className="flex items-center gap-0 group relative">
          <Image
            ...
            className="object-contain transition-all duration-500 logo-glow mix-blend-lighten"
            style={{ height: "auto" }}
            priority
          />
        </a>
      ...
      <div className={`lg:hidden fixed inset-0 top-0 bg-black/95 backdrop-blur-3xl transition-all duration-700 ${mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}>
        <div className="flex flex-col items-center justify-center h-full gap-8">
          {navLinks.map((link, i) => (
            <a
              ...
              className="font-[Space_Grotesk] text-3xl font-bold text-white/70 hover:text-wp-orange transition-all duration-300"
          ...
          <a
            href="#contact"
            onClick={() => setMobileOpen(false)}
            className="btn-premium text-white px-10 py-4 rounded-full text-lg font-semibold mt-4"
          >
            Get a Quote
          </a>
        </div>
